...
class TennisBalle:
    def __init__(self):
        ...
        self.framex = 1280/2
        self.framey = 720/2
        self.speed = 10 #speed measured in pixels

    def move(centerx, centery):
        movementx = self.framex - centerx
        movementy = self.framey - centery
        if(movementx < 0):
            move_right(abs(movementx) / speed) #move camera right code
        elif(movementx > 0):
            move_left(abs(movementx) / speed) #move camera left code
        if(movementy < 0):
            move_down(abs(movementx) / speed) #move camera down code
        elif(movementy > 0):
            move_up(abs(movementx) / speed) #move camera up code

    def random_move():
        move_right(speed) #you can just mess around with this

    def get_variables_of_contour(detected, contour):
